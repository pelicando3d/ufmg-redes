...
void error_msg(errorID ID) {
    switch(ID) {
        ...
        case listening:
            ...
            break;
        case arquivo_null:
            fprintf(stderr, "Problema ao abrir o arquivo\n");
            break;
        default:
            fprintf(stderr, "Erro nao especificado\n", );
            break;
    }
    exit(1);
}
...
void pack(Package *p) {
    /*Trocando o valor do checksum recebido por 0x0000, comparar o cksum gerado, se iguais, valido*/
    uint16_t zero_CHKSUM = 0x0000;     
    ...
    memcpy(&(p->byte_array[8]), &(zero_CHKSUM), sizeof(zero_CHKSUM)); // 64 - 79  (2 bytes) : CHKSUM
...
uint16_t checksum(uint8_t *headers, uint8_t *data, uint16_t len){
    ...
    /* Caso o numero de bytes em data seja impar, nao se faz a ultima soma
        pois o acesso seria fora dos limites reais dos dados no arranjo */
    for (i = 0; i < len; i+=2) {
        unsigned w;
        if(len%2 != 0 && i == len-1) 
            w = data[i];
...
void send_receive_file(char* in_name, char * out_name , int ssocket) {
    ...
    if(input_file == NULL)
        error_msg(arquivo_null); //file doesnt exist      
    ...
    while(true) {          
      ...
      if(!reenviar && !blocked && more_to_send) {
        ...
        //data.DATA[nElem] = '\0';        
        pack(&data);
        data.CHKSUM = checksum(data.byte_array, data.DATA, nElem);
        
        
        if(debug) printf("\n#__envio liberado\n");
        send_package(&data, ssocket);
        if(debug) printf("Enviei %u bytes [%c##%c]\n", data.LENGTH, data.DATA[0], data.DATA[data.LENGTH-2]);                                                
      ...
      if(!more_to_send && !more_to_recv) break;      
        ...
        if(indata.LENGTH > 0 && indata.FLAGS != 0x80) { // Se for pacote de dados
            ...
            uint16_t in_checksum = checksum(indata.byte_array, indata.DATA, indata.LENGTH);

            if(debug) printf("\nin checksum calculado: %x\n", in_checksum);


            //AKI 1
            //if(in_checksum != indata.CHKSUM) exit(1);
            /*Se checksum for valido e ID do recente difere do ultido ID recebido, salva pacote no disco*/
            if(lastID != indata.ID && in_checksum == indata.CHKSUM ) {
                ack.ID = indata.ID;
                ack.ID = indata.ID;
